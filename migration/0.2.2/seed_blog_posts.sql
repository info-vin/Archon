...
### 步驟一：任務啟動 (前端操作)
行銷專員登入 Archon 系統，點擊「新增任務」。他不需要撰寫複雜的提示詞，只需直接上傳 `product_spec.pdf` (包含詳細規格與參數) 和 `copy_draft.txt` (包含推廣標語的初步構想)。接著，他在任務管理面板的「指派給」下拉選單中，選擇了專屬的「設計師 AI Agent」，並設定截止日期後提交。
...
```text
[Human (User/PM)] ─── 1. 建立任務 (建立 UI 工單) ───> [Supabase DB] ─── 2. 異步事件通知 ───> [Archon Supervisor]
       ▲                                                                                     │
       │                                                                                3. 任務拆解 (Star-Topology)
       │                                                                                     │
       │                                                                                     ▼
 10. 批准與執行 ◄─── 9. 比對驗證 Diff ◄─── 8. 產生 UAT 報告與截圖 ◄─── 7. 啟動 Twin Scout ◄─── 4. 分派子任務 (RAG/MCP)
       │                                                                                     │
       └─────────────────────────── [Specialized Agent (Alice/Bob)] ─── 5. 寫入 Proposed Changes ┘
```
...
```text
+------------------------------+---------------+---------------------------------------------+
| 貢獻角色 (開發者/AI/PM)      | 累計 Commit   | 核心貢獻領域                                |
+------------------------------+---------------+---------------------------------------------+
| tek Atrust (核心架構師)      | 935 次 (62%)  | L2 模組化拆分、混沌工程、核心 SDK 重構      |
| Jules & Antigravity (AI)     | 172 次 (11%)  | 自動化修復 (Active Repair Loop)、測試案例生成 |
| info-vin (PM/專案整合)       | 123 次 (8%)   | 規格對齊、SOP 文件化、UAT 測試門禁配置      |
| 其他團隊成員 (Cole, Rasmus 等) | 273 次 (19%)  | 前端組件開發、API 接口設計                  |
+------------------------------+---------------+---------------------------------------------+
```
...
+------------------------------+---------------+---------------+-----------------------------+
| 單元測試通過率               | 89.2%         | 100% (579)    | +10.8% (穩定性提升)         |
| UAT 自動化覆蓋率             | 15.0%         | 85.0%         | +70.0% (人工測試減負)       |
| API 503 錯誤抗性             | 低 (直接當機) | 高 (自癒防禦) | 故障率降至 < 0.1%           |
