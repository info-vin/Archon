...
describe("apiClient (callAPIWithETag)", () => {
  ...
  describe("callAPIWithETag", () => {
    it("should return data for successful request", async () => {
      ...
      // Corrected Assertion: Expect the full URL generated by the environment configuration
      const expectedUrl = `${(import.meta.env as any).VITE_API_BASE_URL}/api/test-endpoint`;
    ...
    it("should handle API errors in response body", async () => {
      ...
      const mockResponse = {
        ok: true,
        status: 200,
        json: () => Promise.resolve(mockData), // Correctly mock json()
        headers: new Headers(),
      };
    ...
    it("should pass custom headers correctly", async () => {
      ...
      // Corrected Assertion: Expect the full URL from environment configuration
      const expectedUrl = `${(import.meta.env as any).VITE_API_BASE_URL}/api/test-endpoint`;
      expect(global.fetch).toHaveBeenCalledWith(
        expectedUrl,
        expect.objectContaining({
