...
export const useOllamaModelFilter = (
  ...
  // PERFORMANCE: Precalculate lowercase names to avoid O(N) string allocations during search filtering
  ...
  const filteredModels = useMemo(() => {
    ...
    const filtered = models.filter((model, index) => {
      ...
      if (searchTerm && !searchableNames[index].includes(searchLower)) return false;
    ...
    return filtered;
  }, [models, searchTerm, compatibilityFilter, sortBy, modelType, selectedInstanceUrl, searchableNames]);
