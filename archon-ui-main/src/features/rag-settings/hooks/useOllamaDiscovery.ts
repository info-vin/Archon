...
export const useOllamaDiscovery = (
  ...
  const saveModelsToCache = useCallback(
    (modelsToCache: EnrichedModel[]) => {
      try {
  ...
  const testModelCapabilities = useCallback(
    async (model: EnrichedModel) => {
      ...
      try {
        ...
        setModels((prev) =>
          prev.map((m) =>
            m.name === model.name && m.instance_url === model.instance_url ? { ...m, status: "error" } : m,
          ),
        );
  ...
  // PERFORMANCE: Precalculate lowercase names to avoid O(N) string allocations during typing
  const searchableNames = useMemo(() => models.map((m) => m.name.toLowerCase()), [models]);

  const filteredAndSortedModels = useMemo(() => {
    const filterTextLower = selectionState.filterText ? selectionState.filterText.toLowerCase() : "";
    const filtered = models.filter((model, index) => {
      if (selectionState.filterText && !searchableNames[index].includes(filterTextLower)) return false;
    ...
    return filtered;
  }, [models, selectionState, searchableNames]);
