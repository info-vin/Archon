  ...
  return (
    <div className="flex h-[600px] gap-6 font-sans">
      <div className="w-64 flex flex-col space-y-4">
        <div className="flex items-center gap-2"><FileText className="w-5 h-5 text-gray-700 dark:text-gray-300" /><h3 className="text-lg font-semibold text-gray-800 dark:text-white">Documents</h3></div>
        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9" /></div>
        <div className="flex-1 space-y-1">
          {filteredDocs.map(d => (
            <button key={d.id} type="button" onClick={() => onDocSelect(d)} className={cn("w-full text-left px-3 py-2 rounded-lg transition-all", d.id === doc.id ? "bg-cyan-500/10 text-cyan-700 border-l-2 border-cyan-500" : "text-gray-600 hover:bg-white/5 border-l-2 border-transparent")}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="text-gray-600 dark:text-gray-400 space-y-4">
          <p>Document type: <span className="px-2 py-1 text-xs bg-blue-500/10 text-blue-600 rounded">{doc.type}</span></p>
