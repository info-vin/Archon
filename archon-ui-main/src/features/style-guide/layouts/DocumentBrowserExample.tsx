...
const DocumentBrowserModal = ({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) => {
  ...
  // PERFORMANCE: Precalculate searchable strings to prevent O(N) string allocations during filtering
  const searchableDocs = useMemo(() => MOCK_DOCUMENTS.map((doc) => doc.title.toLowerCase()), []);
  const searchableCode = useMemo(() => MOCK_CODE.map((code) => code.summary.toLowerCase()), []);
