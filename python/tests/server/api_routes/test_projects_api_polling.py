...
class TestProjectsListPolling:
    def test_list_projects_http_with_etag(self):
        with (
            patch("src.server.api_routes.projects.core.ProjectService") as mock_proj_class,
            patch("src.server.api_routes.projects.core.SourceLinkingService") as mock_source_class,
        ):
            mock_proj_instance = MagicMock()
            mock_proj_instance.list_projects = AsyncMock(
                return_value=(
                    True,
                    {"projects": [{"id": "proj-1", "title": "Test Project", "department": "Engineering"}]},
                )
            )
            mock_proj_class.return_value = mock_proj_instance

            mock_source_instance = MagicMock()
            mock_source_instance.format_projects_with_sources = AsyncMock(
                return_value=[{"id": "proj-1", "title": "Test Project", "department": "Engineering"}]
            )
...
class TestProjectTasksPolling:
    def test_list_project_tasks_http_polling(self):
        ...
        mock_task_service.list_tasks = AsyncMock(
            return_value=(True, {"tasks": [{"id": "task-1", "title": "Test Task", "status": "todo"}]})
        )

        with patch("src.server.api_routes.projects.core.ProjectService") as mock_proj_class:
            mock_proj_instance = MagicMock()
            mock_proj_class.return_value = mock_proj_instance
            mock_proj_instance.get_project = AsyncMock(
                return_value=(True, {"project": {"id": "proj-1", "department": "Engineering"}})
            )
