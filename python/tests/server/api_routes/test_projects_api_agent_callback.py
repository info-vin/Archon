...
def test_report_task_status_from_agent_success():
    ...
    mock_task_service.update_task_status_from_agent = AsyncMock(
        return_value=(
            True,
    ...
    response = client.post(f"/api/tasks/{task_id}/agent-status", json={"status": status, "agent_id": agent_id})
...
def test_report_task_output_from_agent_success():
    ...
    mock_task_service.save_agent_output = AsyncMock(
        return_value=(
            True,
    ...
    response = client.post(f"/api/tasks/{task_id}/agent-output", json={"output": output_data, "agent_id": agent_id})

    assert response.status_code == 200
    assert response.json()["task"]["attachments"][0] == output_data
    mock_task_service.save_agent_output.assert_awaited_once_with(task_id=task_id, output=output_data, agent_id=agent_id)
