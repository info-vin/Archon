...
async def test_process_and_store_documents_logic():
    ...
    with patch(
        "src.server.services.crawling.document_storage_operations.DocumentStorageService"
        ...
        # 模擬外部相依函數
        with patch(
            "src.server.services.crawling.document_storage_operations.add_documents_to_supabase", new_callable=AsyncMock
        ) as mock_add:
