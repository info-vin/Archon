...
async def get_content_sources(current_user: dict = Depends(get_current_user)):
    try:
        ...
        for lead in lead_res.data:
            sources.append({
                "id": lead["id"], "type": "lead", "title": lead["company_name"], 
                "score": lead["enrichment_score"], "summary": lead["identified_need"][:100] if lead["identified_need"] else "", 
        ...
        for task in task_res.data:
            sources.append({
                "id": task["id"], "type": "task", "title": task["title"], "score": 100, 
                "summary": task["description"][:100] if task["description"] else "", "date": task["created_at"]
            })
            
        # 3. Blog Drafts / Returned (GAP-023)
        ...
        for blog in blog_res.data:
            ...
            # If returned, summary MUST be Charlie's reason
            ...
            sources.append({
                ...
                "summary": summary, "date": blog["created_at"], "review_notes": review_notes, 
                "ai_score": blog.get("ai_score"), "status": status
            })
...
async def get_content_context(source_id: str, source_type: str = Query("lead"), current_user: dict = Depends(get_current_user)):
    try:
        ...
        # GAP-023: Backtrack if source is a blog
        if source_type == "blog":
        ...
        if target_type == "lead":
            ...
            if task_res.data:
                ...
                if lead_id:
                    ...
                    for log in l_res.data: context_text += f"\n[Linked Log]: {log.get('summary')}\n{log.get('voice_transcript')}\n"

        rag_refs = []
        if context_text.strip():
            rag_service = RAGService()
            success, result = await rag_service.perform_rag_query(query=context_text[:1000], match_count=3)
            if success and isinstance(result, dict): rag_refs = result.get("results", [])
...
async def get_pending_approvals(current_user: dict = Depends(get_current_user)):
    try:
        ...
        if user_role not in ["manager", "admin", "system_admin"]: raise HTTPException(status_code=403)
    ...
    try:
        ...
        if item_type == "blog":
            ...
            if res.data:
                ...
                if task_id:
                    try: await TaskService(supabase).update_task(task_id, {"status": "done" if action == "approve" else "doing"})
                    except: pass
...
async def submit_blog_for_review(post_id: str, current_user: dict = Depends(get_current_user)):
    try:
        ...
        # PRESERVATION: AI score check, but Charlie's notes are SACRED
        ...
        supabase.table("blog_posts").update({
            "status": new_status, "ai_score": final_score, "review_notes": final_notes
    ...
    try:
        ...
        if request.context_source_id:
            ...
            if request.context_type == "lead":
                ...
                for l in logs.data: context_text += f"\n[Signal]: {l['summary']}\n{l['voice_transcript']}\n"
        ...
        response = client.models.generate_content(
            model="gemini-2.0-flash-lite", contents=cast(Any, [user_prompt]),
...
async def nana_banana_proxy(request: dict, current_user: dict = Depends(get_current_user)):
    ...
    except: return {"status": "fallback", "image_url": "https://picsum.photos/seed/fallback/800/600"}
...
async def get_marketing_trends(current_user: dict = Depends(get_current_user)):
    ...
    except: return {"keyword_growth": [], "sankey_flow": {}}
