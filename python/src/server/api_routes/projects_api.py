"""
Projects API Hardened - Entry point for project-related routes.
Facade pattern with 100% Signature Alignment for Test Compatibility.
Physical RBAC Hardening: All routes now require authentication and check specific scopes.
"""
...
async def list_project_documents(project_id: str, include_content: bool = False, current_user: dict = Depends(get_current_user)):
    s, res = DocumentService().list_documents(project_id, include_content)
    return handle_service_result(s, res)

@router.post("/projects/{project_id}/docs")
async def create_project_document(project_id: str, req: CreateDocumentRequest, current_user: dict = Depends(get_current_user)):
...
async def list_project_versions(project_id: str, field_name: str | None = None, current_user: dict = Depends(get_current_user)):
...
async def create_project_version(project_id: str, req: CreateVersionRequest, current_user: dict = Depends(get_current_user)):
...
async def restore_project_version(project_id: str, field_name: str, version_number: int, req: RestoreVersionRequest, current_user: dict = Depends(get_current_user)):
