...
from ..schemas.agent_outputs import LogEntry, RecordGeminiLogResponse, SystemAlert

router = APIRouter(prefix="/api/logs", tags=["logs"])


@router.post("/record-gemini-log", status_code=status.HTTP_201_CREATED, response_model=RecordGeminiLogResponse)
async def record_gemini_log(
    log_data: dict[str, Any], current_user: UserProfileDTO = Depends(get_current_user)
) -> RecordGeminiLogResponse:
