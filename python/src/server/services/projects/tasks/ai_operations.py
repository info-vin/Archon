    ...
    try:
        ...
        if lead_id:
            ...
            if res_lead.data and len(res_lead.data) > 0:
                ...
                context_str += "COMPANY: {lead['company_name']}\n"
                context_str += "IDENTIFIED NEED: {lead.get('identified_need', 'None')}\n"
        ...
        rag_success, rag_result = await rag_service.perform_rag_query(query="{company_name} {details.get('type', '')}", match_count=2)
        if rag_success and "results" in rag_result:
