...
async def cleanup_system_probes():
    ...
    try:
        ...
        # 1. Leads cleanup
        supabase.table("leads").delete().eq("company_name", "System Probe").lt("created_at", cutoff).execute()

        # 2. Content pages cleanup
        supabase.table("archon_crawled_pages").delete().like("source_id", "pitch-systemprobe-%").lt("created_at", cutoff).execute()

        # 3. Document versions cleanup
        supabase.table("archon_document_versions").delete().eq("created_by", "ai-librarian").like("change_summary", "%System Probe%").lt("created_at", cutoff).execute()

        # 4. Sources cleanup
        supabase.table("archon_sources").delete().like("source_id", "pitch-systemprobe-%").lt("created_at", cutoff).execute()

        logger.info("✅ Clockwork: Cleanup complete.")
    except Exception as e:
