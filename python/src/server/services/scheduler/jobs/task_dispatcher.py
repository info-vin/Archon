...
async def run_task_dispatcher():
    ...
    try:
        ...
        if reclaim_res.data:
            logger.warning(f"🚨 Task Sentinel: Reclaimed {len(reclaim_res.data)} stuck tasks.")
            for t in reclaim_res.data:
                supabase.table("archon_logs").insert({
                    "source": "task-sentinel", "level": "WARNING",
                    "message": f"Auto-reclaimed stuck task: {t['title']}",
                    "details": {"task_id": t["id"], "type": "timeout_reclamation"}
                }).execute()

        # 2. Dispatch recurring tasks
        res = supabase.table("archon_tasks").select("id, title, assignee_id, crawler_target_id").eq("is_recurring", True).eq("status", "todo").execute()
        tasks = res.data or []
        for task in tasks:
            logger.info(f"📡 Clockwork: Dispatching task: {task['title']}")
            await agent_service.run_agent_task(task_id=task["id"], agent_id=task.get("assignee_id", "ai-librarian"))
            ...
            supabase.table("archon_logs").insert({
                ...
                "details": {"task_id": task["id"], "assignee": task.get("assignee_id"), "target_id": task.get("crawler_target_id")}
            }).execute()
