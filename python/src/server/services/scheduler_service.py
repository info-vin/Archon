...
class SchedulerService:
    ...
    async def _should_run_weekly(self, job_id: str, trigger: CronTrigger | None = None) -> tuple[bool, str]:
        last_run = await self._get_last_run(job_id)
        if not last_run:
            return True, ""
    ...
    async def _should_run_monthly(self, job_id: str, trigger: CronTrigger | None = None) -> tuple[bool, str]:
        last_run = await self._get_last_run(job_id)
        if not last_run:
            return True, ""
        last_run_local = last_run.astimezone(DEFAULT_TIMEZONE)
        now_local = datetime.now(DEFAULT_TIMEZONE)
        if (last_run_local.year, last_run_local.month) >= (now_local.year, now_local.month):
            return False, f"Already run this month at {last_run_local.strftime('%Y-%m-%d %H:%M:%S')}"
        return True, ""

    async def _should_run_biweekly(self, job_id: str, trigger: CronTrigger | None = None) -> tuple[bool, str]:
        last_run = await self._get_last_run(job_id)
        if not last_run:
            return True, ""
        last_run_local = last_run.astimezone(DEFAULT_TIMEZONE)
        if (datetime.now(DEFAULT_TIMEZONE).date() - last_run_local.date()).days < 13:
            return False, f"Already run recently at {last_run_local.strftime('%Y-%m-%d %H:%M:%S')} (needs 13 gap days)"
        return True, ""
    ...
    async def _schedule_jobs(self) -> None:
        ...
        await self._schedule_stateful_job(self._run_architecture_health_audit, "architecture_health_audit", 40, self._should_run_weekly, CronTrigger(day_of_week=config.architecture_health_audit_days, hour=health_h, minute=health_m, timezone=DEFAULT_TIMEZONE))

        retro_h, retro_m = self._parse_dynamic_hf_time(config, 1)
        await self._schedule_stateful_job(self._run_engineering_retrospective, "weekly_engineering_retro", 41, self._should_run_weekly, CronTrigger(day_of_week=config.weekly_engineering_retro_days, hour=retro_h, minute=retro_m, timezone=DEFAULT_TIMEZONE))

        await self._schedule_stateful_job(self._run_monthly_executive_summary, "monthly_executive_summary", 42, self._should_run_monthly, CronTrigger(day=config.monthly_summary_day, hour=config.monthly_summary_hour, minute=config.monthly_summary_minute, timezone=DEFAULT_TIMEZONE))

        # --- Category 4: Stateful Bi-weekly Maintenance ---
        await self._schedule_stateful_job(self._run_infrastructure_audit, "infrastructure_audit", 48, self._should_run_biweekly, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=config.maintenance_audit_minute, timezone=DEFAULT_TIMEZONE))
        await self._schedule_stateful_job(self._run_api_deprecation_scan, "api_deprecation_scan", 50, self._should_run_biweekly, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=(config.maintenance_audit_minute + 15) % 60, timezone=DEFAULT_TIMEZONE))
        await self._schedule_stateful_job(self._run_tech_debt_audit, "tech_debt_audit", 45, self._should_run_biweekly, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=(config.maintenance_audit_minute + 30) % 60, timezone=DEFAULT_TIMEZONE))
        await self._schedule_stateful_job(self._run_ssot_audit, "ssot_audit", 47, self._should_run_biweekly, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=(config.maintenance_audit_minute + 45) % 60, timezone=DEFAULT_TIMEZONE))
