...
class SchedulerService:
    ...
    async def _should_run_local_only(self, job_id: str, trigger: CronTrigger | None = None) -> tuple[bool, str]:
        from src.server.config.config import get_config
        config = get_config()
    ...
    async def _should_run_opportunistic_weekly_local_only(self, job_id: str, trigger: BaseTrigger | None = None) -> tuple[bool, str]:
        from src.server.config.config import get_config
        config = get_config()
        if config.space_id is not None:
            return False, "Skipped in remote cloud environment"
        return await self._should_run_opportunistic_weekly(job_id, trigger=trigger)
    ...
    async def _should_run_daily(self, job_id: str, trigger: CronTrigger | None = None) -> tuple[bool, str]:
        ...
        if last_run.astimezone(DEFAULT_TIMEZONE).date() >= now_local.date():
            return False, f"Already run today at {last_run.astimezone(DEFAULT_TIMEZONE).strftime('%H:%M:%S')}"
        return True, ""

    async def _should_run_opportunistic_weekly(self, job_id: str, trigger: BaseTrigger | None = None) -> tuple[bool, str]:
        last_run = await self._get_last_run(job_id)
        if not last_run:
            return True, ""
        ...
        if last_run_shifted.isocalendar()[:2] >= now_shifted.isocalendar()[:2]:
            return False, f"Already run this week at {last_run.astimezone(DEFAULT_TIMEZONE).strftime('%Y-%m-%d %H:%M:%S')}"
        return True, ""

    async def _should_run_scheduled_job(self, job_id: str, trigger: BaseTrigger | None = None) -> tuple[bool, str]:
        last_run = await self._get_last_run(job_id)
        if not last_run:
            return True, ""
        ...
        if trigger:
            try:
                next_expected = trigger.get_next_fire_time(last_run_local, last_run_local)
                if next_expected and now_local < next_expected:
                    return False, f"Not reached next schedule {next_expected.strftime('%Y-%m-%d %H:%M:%S')}"
            except Exception:
                pass
    ...
    async def _schedule_stateful_job(self, job_func: Callable[..., Any], job_id: str, delay_mins: int, check_func: Callable[..., Any], trigger: CronTrigger) -> None:
        if not self._scheduler:
            return
        async def wrapper() -> None:
            should_run, reason = await check_func(job_id, trigger=trigger)
            if should_run:
                ...
                logger.info(f"⏭️ Clockwork: Skipped '{job_id}' ({reason})")
    ...
    async def _schedule_jobs(self) -> None:
        ...
        health_h, health_m = self._parse_dynamic_hf_time(config, offset_hours=3)
        await self._schedule_stateful_job(self._run_architecture_health_audit, "architecture_health_audit", 40, self._should_run_scheduled_job, CronTrigger(day_of_week=config.architecture_health_audit_days, hour=health_h, minute=health_m, timezone=DEFAULT_TIMEZONE))

        retro_h, retro_m = self._parse_dynamic_hf_time(config, offset_hours=4)
        await self._schedule_stateful_job(self._run_engineering_retrospective, "weekly_engineering_retro", 41, self._should_run_scheduled_job, CronTrigger(day_of_week=config.weekly_engineering_retro_days, hour=retro_h, minute=retro_m, timezone=DEFAULT_TIMEZONE))

        await self._schedule_stateful_job(self._run_monthly_executive_summary, "monthly_executive_summary", 42, self._should_run_scheduled_job, CronTrigger(day=config.monthly_summary_day, hour=config.monthly_summary_hour, minute=config.monthly_summary_minute, timezone=DEFAULT_TIMEZONE))

        # Category 4: Bi-weekly maintenance
        await self._schedule_stateful_job(self._run_infrastructure_audit, "infrastructure_audit", 48, self._should_run_scheduled_job, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=config.maintenance_audit_minute, timezone=DEFAULT_TIMEZONE))
        await self._schedule_stateful_job(self._run_api_deprecation_scan, "api_deprecation_scan", 50, self._should_run_scheduled_job, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=(config.maintenance_audit_minute + 15) % 60, timezone=DEFAULT_TIMEZONE))
        await self._schedule_stateful_job(self._run_tech_debt_audit, "tech_debt_audit", 45, self._should_run_scheduled_job, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=(config.maintenance_audit_minute + 30) % 60, timezone=DEFAULT_TIMEZONE))
        await self._schedule_stateful_job(self._run_ssot_audit, "ssot_audit", 47, self._should_run_scheduled_job, CronTrigger(day_of_week=config.maintenance_audit_days, hour=config.maintenance_audit_hour, minute=(config.maintenance_audit_minute + 45) % 60, timezone=DEFAULT_TIMEZONE))
