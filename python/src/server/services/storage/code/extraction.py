...
def extract_code_blocks_logic(markdown_content: str, min_length: int | None = None) -> list[dict[str, Any]]:
    ...
    while i < len(backtick_positions) - 1:
        ...
        if not language or language in ["text", "plaintext", "txt"]:
            ...
            if enable_diagram_filtering:
                ...
                special_char_lines = sum(1 for line in non_empty_lines[:10] if line and sum(1 for c in line if not c.isalnum() and not c.isspace()) / len(line) > 0.7)
                diagram_indicator_count = sum(1 for ind in _DIAGRAM_INDICATORS if ind in code_content)
    ...
    if not code_blocks: return code_blocks
    ...
    for idx, block1 in enumerate(code_blocks):
        ...
        for jdx, block2 in enumerate(code_blocks):
            if jdx <= idx or jdx in processed_indices: continue
