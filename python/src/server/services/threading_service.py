...
class ThreadingService:
    ...
    async def batch_process(self, items: list[Any], process_func: Callable, mode: ProcessingMode = ProcessingMode.CPU_INTENSIVE, progress_callback: Callable | None = None) -> list[Any]:
        return await self.memory_dispatcher.process_with_adaptive_concurrency(items, process_func, mode, progress_callback)
    ...
    async def _health_check_loop(self):
        while self._running:
            try:
                ...
                logfire_logger.info("System health check", extra={"memory_percent": metrics.memory_percent, "cpu_percent": metrics.cpu_percent, "available_memory_gb": metrics.available_memory_gb, "active_threads": metrics.active_threads})

                if metrics.memory_percent > 90:
                    gc.collect()
                    try:
                        from .log_service import log_service
                        log_service.create_log_entry({"project_name": "system-resource", "gemini_response": f"CRITICAL: Memory usage at {metrics.memory_percent}%", "user_input": "Automatic Health Check"})
                    except Exception:
                        pass
                ...
                if metrics.cpu_percent > 95:
                    try:
                        from .log_service import log_service
                        log_service.create_log_entry({"project_name": "system-resource", "gemini_response": f"CRITICAL: CPU usage at {metrics.cpu_percent}%", "user_input": "Automatic Health Check"})
                    except Exception:
                        pass
