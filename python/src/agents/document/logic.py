...
def _convert_to_blocks(title: str, document_type: str, content_description: str) -> list[dict[str, Any]]:
    """Helper to convert content to block-based format."""
    blocks = []
    blocks.append({"id": str(uuid.uuid4()), "type": "heading_1", "content": title, "properties": {"text": title}})
    blocks.append({"id": str(uuid.uuid4()), "type": "heading_2", "content": "Overview", "properties": {"text": "Overview"}})
    blocks.append({"id": str(uuid.uuid4()), "type": "paragraph", "content": content_description, "properties": {"text": content_description}})
    return blocks
