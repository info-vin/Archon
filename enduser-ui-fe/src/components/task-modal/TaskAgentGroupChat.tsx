...
import { audioMarkdownComponents } from '../AudioMarkdownRenderer.tsx';
      ...
      return (
        <div key={idx} className={`flex w-full mb-4 ${isRight ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
          ...
          <div className={`max-w-[75%] p-3 rounded-xl ${config.color} shadow-sm border border-black/5 dark:border-white/5`}>
            ...
            <div className="text-sm break-words prose prose-sm prose-indigo max-w-none dark:prose-invert leading-tight">
              <ReactMarkdown components={audioMarkdownComponents}>{msg.content}</ReactMarkdown>
            </div>
  ...
  if (messagesLength === 0) {
    return (
      <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-xl space-y-3 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="bg-white dark:bg-slate-950 p-4 rounded-lg border border-indigo-50 dark:border-indigo-900 text-sm text-slate-700 dark:text-slate-300 overflow-x-auto max-h-60 custom-scrollbar prose prose-sm prose-indigo max-w-none dark:prose-invert">
          <ReactMarkdown components={audioMarkdownComponents}>{output.content || output.final_result || JSON.stringify(output, null, 2)}</ReactMarkdown>
        </div>
