...
import { PlusIcon, XIcon, CheckCircleIcon, KeyIcon } from '../../../components/Icons.tsx';
...
const EditUserModal: React.FC<{ user: Employee; onClose: () => void; onSave: (updatedUser: Employee) => void; }> = ({ user, onClose, onSave }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            onSave(updatedUser);
            alert('User updated successfully and credentials synced!');
            onClose();
        } catch(error: any) {
         ...
         <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl shadow-xl w-full max-w-lg p-6 relative animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="text-2xl font-bold italic">Manage Access: {user.name}</h2>
                </div>
                ...
                <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="role" className="block text-xs font-bold uppercase text-muted-foreground mb-1">Role Assignment</label>
                    ...
                    <div className="bg-muted/30 p-4 rounded-xl border border-border">
                        <h4 className="text-xs font-bold uppercase text-muted-foreground mb-3">Effective Capabilities (based on Role)</h4>
                        <div className="flex flex-wrap gap-2">
                            {(ROLE_PERMISSIONS_MAP[role.toLowerCase()] || ROLE_PERMISSIONS_MAP['viewer']).map(p => (
                                <span key={p} className="px-2 py-1 bg-background border border-border rounded text-[10px] font-mono text-primary flex items-center gap-1">
                    ...
                    <div className="flex justify-end space-x-2 pt-4 border-t border-border mt-4">
                        ...
                        <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition-all font-bold disabled:opacity-50">
                            {isSaving ? 'Syncing...' : 'Save & Sync Metadata'}
                        </button>
                    </div>
                </div>
            </div>
