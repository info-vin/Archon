...
const safeFormatVisitDate = (dateVal: any, formatter: Intl.DateTimeFormat) => {
    const d = new Date(dateVal);
    return isNaN(d.getTime()) ? 'Invalid Date' : formatter.format(d);
};
                                        ...
                                        {summary.scheduling_recommendation.suggested_slots.map((slot: any, idx: number) => {
                                            ...
                                            return (
                                                <div key={idx} className="p-2 bg-purple-50 text-purple-900 border border-purple-100 rounded text-xs">
                                                    Slot {String.fromCharCode(65 + idx)}: {safeFormatVisitDate(st, dateFormatter)} {safeFormatVisitDate(st, timeFormatter)} ~ {safeFormatVisitDate(et, timeFormatter)}
                                                </div>
                                            );
                                        })}
                                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
            <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-xl w-full max-w-md p-6 animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button type="button" onClick={onClose} aria-label="Close modal" className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-full"><XIcon className="w-6 h-6 text-gray-400" /></button>
                </div>
