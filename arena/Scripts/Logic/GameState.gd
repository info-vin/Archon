...
var effect_resolver: RefCounted
...
func _init(mana: int = 5, hp: int = 100):
	...
	effect_resolver = preload("res://Scripts/Logic/CardEffectResolver.gd").new()
...
func play_card(card: CardStats, _enemy: RefCounted = null) -> bool:
	...
	var effect_msg = effect_resolver.resolve_effects(card, self, final_damage)
	msg += effect_msg
