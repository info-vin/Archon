...
> 本章節提煉了從大量歷史日誌中反覆出現的、最有價值的經驗教訓，並將其歸納為可複用的偵錯模式。

### 核心偵錯哲學 (Core Debugging Philosophies)
...
### 常見場景模式 (Common Scenario Patterns)
...
*   **測試非同步與單例服務的「黃金模式」 (Ref: 2025-12-20)**
    *   **現象**: 測試 FastAPI 端點時，若其依賴一個在 `app` 啟動時就已初始化的服務單例 (singleton)，使用標準的 `pytest.fixture` 進行 `patch` 會失敗。
    *   **模式**: 必須在 `import app` **之前**就定義好 `patch`，並在模組級別使用 `setup_module` / `teardown_module` 手動 `start()` 和 `stop()`。這是測試此類架構的唯一可靠方法。同時，應警惕**應用程式自身 Bug**，例如一個同步函式錯誤地呼叫了非同步函式卻沒有 `await`。

*   **大型重構的風險管理 (Ref: 2025-12-12)**
    *   **現象**: 一個大型重構（例如，為服務增加故障轉移功能）導致了大量的測試失敗。
    *   **模式**:
        1.  **追溯根源**: 使用 `git log -p <file>` 快速定位引入破壞性變更的**單一**提交。
        2.  **區分 Mock 類型**: 在修復測試時，要極度小心 `Mock`（同步）和 `AsyncMock`（非同步）的區別，誤用會導致 `TypeError`。
        3.  **務實處理**: 對於不成比例的、頑固的非核心 Bug，使用 `@pytest.mark.xfail` 是一個記錄問題、同時確保主流程不受阻塞的有效工具。

*   **部署後驗證的「最後一哩路」 (Ref: 2025-12-15, 2025-12-27)**
    *   **現象**: 前端部署成功，但出現樣式錯誤或資料不一致；或者，合併分支後，E2E 測試失敗。
    *   **模式**:
        1.  **`commit` ≠ `push`**: 一個修復只有在被成功 `push`、建置、部署，並由**終端使用者**在瀏覽器中驗證後，才算「完成」。
        2.  **警惕客戶端狀態**: 前端問題可能源於**客戶端**的環境配置。例如，資料不一致可能是因為瀏覽器 `localStorage` 缺少 API 金鑰，觸發了假資料降級機制。
        3.  **精確的合併衝突解決**: 合併衝突後，不能僅依賴 IDE 的自動合併。必須使用 `git diff <target-branch> -- <file>` 來手動驗證最終的程式碼，是否與**目標分支**的預期完全一致。

*   **挖掘真實的專案願景 (Ref: 2025-12-16)**
    *   **現象**: 對專案的下一步感到困惑，或架構圖與核心目標看似不符。
    *   **模式**: 專案的真實「願景」可能隱藏在看似不起眼的**假資料**（例如 `MOCK_BLOG_POSTS` 的內容）、PRP 文件或使用者故事中。必須進行跨檔案的深度歷史分析，才能理解程式碼背後的真實意圖，並區分「已部署的架-構」和「待實現的協作體驗」。

*   **警惕複合型 Bug：一個頂層指令失敗的背後 (Ref: 2025-12-29)**
    *   **現象**: 一個高階的、涵蓋多個子專案的指令（如 `make test`）失敗，但初步的調查似乎無法找到單一的根本原因。
    *   **模式**: 這通常是一個「複合型 Bug」的信號。其背後可能隱藏著**多個、性質完全不同**的底層 Bug。例如，一個 Bug 可能在 `archon-ui-main` 的元件邏輯中，而另一個 Bug 可能在根目錄的 `Makefile` 指令稿本身。必須採用系統性的隔離偵錯方法，逐一驗證每個子專案自己的測試指令（如 `pnpm run test:e2e`），而不是反覆執行頂層指令。
