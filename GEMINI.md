...
* **15. 拒絕幽靈降級、魔術數字與盲猜 (Zero Silent Fallback, Hardcoding, & Guessing)**
    * **核心**: 鑑於 2026 年 8、9 月反覆發生的結構性錯誤，特立此鐵律：
        1. **禁止靜默吞異常**：絕不允許 `except Exception: pass`。任何異常必須留下 `logger.warning` 供追蹤。
        2. **嚴守 SSOT 與 DRY**：禁止為了解決眼前問題而在代碼中散落時區、重試時間或預設 Prompt 等「魔術數字」。
        3. **敬畏非同步邊界**：在 `async` 函式中嚴禁呼叫會引發 `time.sleep()` 阻塞的同步 SDK，防範排程死鎖。
        4. **禁止跳躍歸因與盲猜**：遇到報錯時，必須依據日誌與歷史文件（如 `Makefile` 歷史）物理追蹤，絕不憑空幻想「樂觀路徑」或丟給使用者做盲目選擇。
        5. **探針與公證先行**：修復代碼後必須同步產出防禦性的測試斷言或探針，杜絕「肉眼驗證」的虛假開發。
