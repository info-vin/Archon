...
### 本日會話總結與學習教訓 (2025-12-24)
*   **核心任務**: 系統性地解決前端 `archon-ui-main` 專案中大量的 ESLint 警告，將程式碼品質恢復到一個健康的基準線。
*   **偵錯與修復歷程**:
    1.  **初始狀態**: 開始時，專案存在約 160 個 Lint 警告，主要分為兩大類。
    2.  **第一階段 (`react/no-unescaped-entities`)**: 首先，我集中處理了所有「未轉義的 HTML 實體」警告（例如 `"` 應為 `&quot;`）。此階段順利完成，解決了所有可修復的實例。
    3.  **第二階段 (`@typescript-eslint/no-unused-vars`)**: 接著，我開始處理最大宗的「未使用的變數」警告。此過程充滿挑戰，因為 `eslint --fix` 的自動修復會不斷改變檔案狀態，導致我多次基於過時的報告進行操作，從而引發了「改A壞B」的錯誤（例如意外移除了 `Server` 元件的導入）。
    4.  **確立「單點驗證」工作流**: 在您的指導下，我最終確立了**「一次只修復一個檔案 -> 單獨驗證 -> 成功後再繼續」**的穩定工作流程。這被證明是唯一可靠、能確保進度且避免混淆的方法。
    5.  **處理頑固的「誤報」**: 在過程中，我們發現了幾個 `eslint-disable` 註解無法成功抑制的持續性誤報（主要是在 `catch (error)` 區塊）。我們將其標記為「已知問題」，暫時擱置以推進整體進度。
*   **最終成果**:
    *   成功將 ESLint 警告總數從 **約 160 個** 大幅減少至 **30 個**。
    *   所有 `react/jsx-no-undef` 的**錯誤 (Error)** 都已被清除。
*   **關鍵學習**:
    *   **信任但驗證，尤其是對 Linter**: 在大規模 Lint 修復任務中，`eslint --fix` 會使警告報告快速過時。唯一可靠的「事實來源」是**當下立即執行**的 Lint 檢查結果。
    *   **工作流的重要性**: 「小步快跑，單點驗證」的工作流雖然看似緩慢，但在複雜的修復任務中，是避免混亂、確保進度最有效率的方法。
    *   **務實主義**: 對於無法輕易解決的、疑似 Linter 工具本身的誤報，應果斷將其記錄並暫時擱置，而不是無止盡地投入時間，阻塞主要任務。

*   **剩餘未完成項目統計**:
    | 警告類型 | 數量 | 主要檔案 | 狀態 |
    | :--- | :--- | :--- | :--- |
    | `react/no-unescaped-entities` | 1 | `IDEGlobalRules.tsx` | 已知頑固誤報 |
    | `@typescript-eslint/no-unused-vars` | 18 | `Ollama...`, `RAGSettings.tsx` | 大部分為 `catch` 區塊的誤報 |
    | `react/display-name` | 5 | 多個 `*.test.ts` 檔案 | 待處理 |
    | `react-hooks/exhaustive-deps` | 3 | `useInspectorData.ts`, `useToast.ts` | 待處理 |
    | `@typescript-eslint/no-empty-object-type`| 3 | `data-card.tsx` | 待處理 |
