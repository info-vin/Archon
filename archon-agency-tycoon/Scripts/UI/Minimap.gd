...
func update_minimap(office_grid: Node2D, agent_manager: Object, agent_views: Dictionary) -> void:
	...
	var grid_size = Vector2(1600, 1200)
	if "size" in office_grid:
		grid_size = office_grid.size
	var content_x = max(grid_size.x, 800.0)
	var content_y = max(grid_size.y, 600.0)
	...
	for room in office_grid.get_children():
		...
		var room_size = Vector2(512, 512)
		if "size" in room: room_size = room.size
		final_node.size = room_size * uniform_scale
