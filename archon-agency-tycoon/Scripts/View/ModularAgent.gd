...
func _setup_spritesheet_animations() -> void:
	var anim_player = $AnimationPlayer
	if anim_player:
		var helper = preload("res://Scripts/View/AgentAnimationHelper.gd").new()
		helper.setup_spritesheet_animations(anim_player)
...
func apply_agent_data(agent_data: AgentResource) -> void:
	...
	if agent_data.spritesheet_path != "":
		is_bob = false
		var tex = ResourceLoader.load(agent_data.spritesheet_path, "Texture2D", ResourceLoader.CACHE_MODE_REPLACE)
		if tex:
			body_sprite.texture = tex
			body_sprite.region_enabled = false
			body_sprite.hframes = 26
			...
			if outfit_sprite:
				...
				outfit_sprite.region_enabled = false
			if tool_sprite:
			...
			_setup_spritesheet_animations()
			
			if energy_bar:
				energy_bar.value = agent_data.energy
				if agent_data.energy > 50:
					energy_bar.modulate = Color(0.2, 1, 0.2)
				elif agent_data.energy > 25:
					energy_bar.modulate = Color(1, 1, 0.2)
				else:
