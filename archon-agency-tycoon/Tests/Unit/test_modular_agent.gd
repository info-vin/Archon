...
func test_spritesheet_option_c() -> void:
	var scene = load("res://Scenes/Main/ModularAgent.tscn")
	var view = scene.instantiate()
	var root = tree.root
	root.add_child(view)
	...
	var agent = preload("res://Scripts/Resources/AgentResource.gd").new("SpritesheetAgent", 1) # DEV
	agent.spritesheet_path = "res://Assets/Characters/char_sheet_v2.png"
	
	view.apply_agent_data(agent)
	
	assert_not_null(view.body_sprite.texture, "Spritesheet texture should be loaded on body_sprite")
	assert_eq(view.body_sprite.hframes, 26, "hframes should be configured to 26")
	assert_eq(view.body_sprite.vframes, 1, "vframes should be configured to 1")
	assert_eq(view.eyes_sprite.texture, null, "Eyes layer texture should be null")
	assert_eq(view.hair_sprite.texture, null, "Hair layer texture should be null")
	assert_eq(view.outfit_sprite.texture, null, "Outfit layer texture should be null")
	assert_eq(view.tool_sprite.texture, null, "Tool layer texture should be null")
