...
*   **測試目標**: 驗證圖的發散與聚合邏輯正確。
*   **斷言要求**:
    *   `beta_graph.run()` 能成功觸發。
    *   Mock 被呼叫的次數必須為 4 次 (3 個 Worker + 1 個 Supervisor)。
    *   最終 `BetaState.map_results` 字典確實包含 `sales`, `marketing`, `system` 三把 Key。
    *   `SharedState.final_result` 確實有產出非空的最終摘要字串。
...
*   **斷言要求**:
    *   執行過程不能拋出 Unhandled Exception 導致整個 Test Case 崩潰。
    *   最終的 `map_results['system']` 必須優雅降級為包含錯誤訊息的字串 (e.g., `"Failed due to error..."`)。
    *   `sales` 與 `marketing` 的正常資料必須保留不受影響。
...
*   **斷言要求**: 
    *   執行結束後，`SharedState.input_tokens` 必須精確等於 `40` (3 Worker * 10 + 1 Supervisor * 10)。
    *   證明全域狀態具備併發寫入的安全性。
...
*   **行為**: 
    1. 實體化 `SharedState`，並呼叫 `beta_graph.run()`。
    2. 將產出的 `final_result` (Executive Summary) 寫入資料庫的 `tasks` 表。
    3. 將該 Task 指派給 Supervisor/Manager 角色 (Charlie)，標題設定為 `[Daily Report] Executive Summary`。

### 2. 排程器自動化測試
*   **位置**: `python/tests/services/test_business_scheduler.py`
*   **測試目標**: 確保業務邏輯確實能將 Graph 的記憶體輸出，轉換為資料庫中的持久化實體。
*   **斷言要求**: 
    *   Mock `beta_graph.run()` 以及 `supabase.table("tasks").insert()`。
    *   驗證 `insert` 方法有被正確呼叫，且 Payload 中包含正確的標題與 Graph 輸出的內容。
...
*   **行為**: 在 `SchedulerService._schedule_jobs()` 中，使用 `_schedule_stateful_job` 將 `run_daily_executive_summary` 加入排程。
*   **頻率設定**: 週期設定為 **24 小時 (Daily)**，並確保具備重啟狀態記憶 (Stateful)，防止伺服器重啟導致漏發或重複派發。

### 2. 終極物理驗證 (Twin Scout - Playwright)
*   **行動**: 修改或新增一個 Playwright E2E 測試腳本 (例如透過 `make twin-scout` 機制)。
*   **斷言**: 由 Playwright 啟動無頭瀏覽器，登入為 Manager (Charlie)。導航至 Task 列表，**物理斷言 (Physical Assertion)** DOM 中確實能渲染出這張 `[Daily Report]` 的卡片，達成從後端引擎到前端 React 渲染的 100% 物理對齊。
