...
| 級別 | 類別 | 範圍 | 晉升要求 (成功次數) |
| :--- | :--- | :--- | :--- |
| **L1** | **微手術** | Lint 錯誤、單行修復。 | **起點 (> 500 次成功)** |
| **L2** | **小重構** | 300 行以下函式修正。 | **L1 達標 + 100 次成功** |
| **L3** | **邏輯剝離** | SQL 搬移至 Service。 | **L2 達標 + 50 次成功** |
| **L4** | **接口調整** | 修改 API 參數或結構。 | **L3 達標 + 20 次成功** |
| **L5** | **依賴解耦** | 修復循環引用與依賴。 | **L4 達標 + 10 次成功** |
| **L6** | **服務演進** | 跨模組數據流變更。 | **L5 達標 + 5 次成功** |
| **L7** | **核心變革** | Base 基座或安全原則。 | **需使用者手動特權授權** |
