...
- [ ] **0.1 Verify Migration 022**: Ensure `migration/022_add_blog_lead_relation.sql` exists and contains RLS policies for `leads` and `visit_logs` (Score >= 80).
- [ ] **0.2 Verify Makefile**: Confirm `make db-init` is available.
- [ ] **0.3 Configure Settings**:
...
- [ ] **1.1.1 Add `GET /api/marketing/sources`**:
...
- [ ] **1.1.2 Add `GET /api/marketing/context/{source_id}`**:
    - **Logic**:
...
- [ ] **1.1.3 Update `POST /api/marketing/drafts`**:
    - **Param**: Accept `context_source_id` and `context_type`.
    - **Logic**: Inject retrieved context into `MarketBot` prompt.
- [ ] **1.1.4 Add `POST /api/marketing/nana-banana`**:
    - **Logic**: Proxy to Google Gemini Vision/Imagen.
    - **Model**: Fetch `NANA_BANANA_MODEL` from settings.

### 1.2 Service Layer Logic
- [ ] **1.2.1 Update `LibrarianService`**: Ensure `search` accepts generic text queries (not just `lead_id`).
- [ ] **1.2.2 Update `AgentRegistry`**: Register "Nana Banana" tool config (if not present).

## 2. Frontend Implementation (`enduser-ui-fe`)

### 2.1 Layout & Navigation
- [ ] **2.1.1 Refactor `BrandPage.tsx`**:
...
- [ ] **2.2.1 Create `<ContentSourceList />`**:
...
- [ ] **2.3.1 Create `<WorkbenchTabs />`**:
    - Tabs: `Context` (Read-Only) | `Editor` (Write).
- [ ] **2.3.2 Create `<ContextViewer />`**:
...
- [ ] **2.3.3 Create `<MagicEditor />`**:
    - Toolbar: `✨ Draft`, `🎨 Image`, `💾 Save`, `🚀 Publish`.
    - State: `draftContent` (Markdown).

### 2.4 Integration
- [ ] **2.4.1 Wire `useContentSources` hook**: Fetch from `/api/marketing/sources`.
- [ ] **2.4.2 Wire `useContextLoader` hook**: Fetch on source selection.
- [ ] **2.4.3 Wire Agent Actions**:
