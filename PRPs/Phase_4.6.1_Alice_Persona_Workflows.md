...
**針對「下午茶時段」 (Enrichment Loop) 的實作釐清：**

1.  **進入檢查模式**:
    *   在桌面版或行動版的 Dashboard 中，點擊 **"Review Queue"** 篩選按鈕（位於表格上方）。
    *   這會篩選出所有狀態為 `new` 或 `pending` 且需要人工確認的潛在客戶。

2.  **檢視 AI 成果 (AI Insights)**:
    *   點擊 **"Promote" (晉升)** 按鈕。這不僅是晉升動作，更是**驗收關卡**。
    *   在彈出的視窗中，系統會顯示 **"AI Insights (Auto-Enriched)"** 區塊，列出 AI 自動抓取的：
        *   **統一編號 (Tax ID)**
        *   **聯絡信箱 (Email)** (會自動填入表單)
        *   **近期新聞摘要 (News)**
    
3.  **歸檔與參數**:
    *   系統預設會自動剔除 3 天內未處理且品質低劣的 Leads。
    *   若需測試此功能，可在 `.env` 設定 `PRUNING_THRESHOLD_MINUTES=10` 來加速歸檔觸發。
