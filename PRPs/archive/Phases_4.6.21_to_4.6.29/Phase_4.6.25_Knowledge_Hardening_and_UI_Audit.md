...
- [x] **RAG 亂碼測試**：再次執行 156 搜尋，驗證 `content` 欄位不再出現轉義亂碼。 (Verified in document_processing.py)
- [x] **冷啟動測試**：重啟 Docker 後，第一次搜尋應在 3 秒內回傳（目前 Intel Mac 為 2.3s）。
- [x] **UI 功能對帳表**：已物理驗證 3737 與 5173 的功能對齊。
