...
## 2026-09-20 - Avoid micro-optimizations that degrade performance
**Learning:** Optimizing an `Array.find` call inside a user interaction handler (like a button click) by extracting it into a `React.useMemo` backed `Map` is an anti-pattern when the data is frequently updated but the interaction is infrequent (a cold path). It forces the application to perform `O(N)` work and allocate memory for a Map on every render/data update, just to speed up an `O(N)` traversal that takes nanoseconds and only happens occasionally. This is a micro-optimization that actually degrades performance.
**Action:** Do not attempt to optimize `Array.find()` lookups by dynamically building a `Map` inside a render cycle or `useMemo` block if the lookup only happens on infrequent user interactions. Only use O(1) Map lookups if the Map can be statically defined outside the component, or pre-computed completely outside the render cycle, or if the lookup happens in a hot path (e.g., inside a `.map` loop during rendering).
