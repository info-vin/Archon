...
## 2024-05-19 - Avoid string allocation in truthiness checks
**Learning:** In Python list comprehensions filtering strings, using `if line and not line.isspace()` is significantly faster than using `if line.strip()` to exclude empty or whitespace-only lines because it avoids creating new stripped string objects.
**Action:** Always prefer `if line and not line.isspace()` over `if line.strip()` when filtering lines in hot paths.
