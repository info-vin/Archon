...
## 2024-05-14 - Expanding Scope of ARIA Context Updates
**Learning:** When making `aria-label`s on icon-only buttons dynamic to include specific entity titles (e.g., `projectTitle` or `taskTitle`), it is important to ensure these title props are actually defined and passed in the component interface, and to check that they are populated correctly by the parent. Also, ensure other closely related generic labels in the same block, such as tooltips (e.g., `content="Delete task"`), are updated consistently with the new specific labels.
**Action:** Before changing a generic label to a specific one using a variable, use tools like `cat` or `sed` to verify the variable exists in the component scope. When updating an `aria-label`, also check if wrapping tooltips or titles need the same context.
